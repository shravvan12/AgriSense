...
def add_custom_css():
    st.markdown("""
        ...
        section.main h5, section.main h6, section.main p, section.main span, section.main div {
        ...
        /* Fix selectbox text color (input and dropdown options) */
...
if choice == "Home":
    ...
    st.markdown("Enter the following to get a fertilizer recommendation:")
    ...
    if uploaded_file is not None:
        ...
        try:
            # Load trained model
            model = tf.keras.models.load_model("artifacts/disease/plant_disease_model.h5")

            # Predict
            predictions = model.predict(image_array)
            predicted_class = np.argmax(predictions, axis=1)[0]

            # Define your class names (in same order used during training)
            class_names = [
